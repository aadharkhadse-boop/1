...
function Dashboard({ password, onLock }: { password: string; onLock: () => void }) {
  const { vesselsByMonth, ciiByMonth, meta, loading, loadingText, error, reload } = useFleetData(password);
  const [tab, setTab] = useState<TabKey>('dashboard');
  const [selected, setSelected] = useState<Selected>(null);
  // Which reporting month the user is viewing. `null` tracks the latest month automatically until
  // the user picks one explicitly from the header dropdown.
  const [selMonth, setSelMonth] = useState<number | null>(null);

  const monthLabels = meta?.monthLabels ?? [];
  const lastIdx = monthLabels.length - 1;
  const monthIdx = selMonth == null ? lastIdx : Math.min(Math.max(selMonth, 0), lastIdx);

  // The vessel list + CII rows for the selected month only. The vessel drawers read from these
  // same objects, so a drawer opened while viewing an earlier month shows that month's figures —
  // the drawer components themselves are unchanged.
  const vessels = (vesselsByMonth && monthIdx >= 0) ? (vesselsByMonth[monthIdx] ?? null) : null;
  const ciiRows = (ciiByMonth.length && monthIdx >= 0) ? (ciiByMonth[monthIdx] ?? []) : [];
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', color: '#003143' }}>
      <Header monthLabels={monthLabels} selectedMonth={monthIdx} onSelectMonth={setSelMonth} />
